...
SENSITIVE_KEYS = {'credentials', 'tt_credentials', 'tradier_credentials',
                  'tradier_sandbox',
                  'finnhub_key', 'admin_token_hash', 'auth_user'}
...
class Database:
    ...
    def create_snapshot(self, label: str = "auto"):
        ...
        snap = {
            ...
            "label": label,
            "data": {
                "journal_tradier_live":    self._get("journal_tradier_live", []),
                "journal_tradier_sandbox": self._get("journal_tradier_sandbox", []),
                "journal_tt_live":         self._get("journal_tt_live", []),
                "journal_tt_sandbox":      self._get("journal_tt_sandbox", []),
                "closed_trades_permanent": self._get("closed_trades_permanent", []),
                "lists":                   self._get("lists", {}),
                "settings":                self._get("settings", {}),
            }
        }
        snaps = self._get("snapshots", [])
        snaps.append(snap)
        # Rotate: keep last 10
        if len(snaps) > 10:
            snaps = snaps[-10:]
        self._set("snapshots", snaps)
        return snap["ts"]

    def list_snapshots(self):
        snaps = self._get("snapshots", [])
        return [{"ts": s["ts"], "label": s.get("label", "auto"),
                 "trades_count": sum(len(s["data"].get(k, [])) for k in s["data"] if k.startswith("journal_")),
                 "closed_count": len(s["data"].get("closed_trades_permanent", []))}
                for s in snaps]

    def restore_snapshot(self, ts: str):
        snaps = self._get("snapshots", [])
